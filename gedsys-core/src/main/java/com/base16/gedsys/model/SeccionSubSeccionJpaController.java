...
public class SeccionSubSeccionJpaController implements Serializable {
    ...
    public List<SeccionSubSeccion> findSeccionSubSeccionByDependencia(SeccionSubSeccion seccionSubSeccion) {
        EntityManager em = getEntityManager();
        try {
            CriteriaQuery cq = em.getCriteriaBuilder().createQuery();
            cq.select(cq.from(SeccionSubSeccion.class));
            Query q = em.createNamedQuery("SeccionSubSeccion.findByDependeDe", SeccionSubSeccion.class)
                    .setParameter("dependeDe", seccionSubSeccion);
            return q.getResultList();
        } finally {
            em.close();
        }
    ...
    public List<SeccionSubSeccion> findSeccionSubSeccionRoots() {
        EntityManager em = getEntityManager();
        try {
            CriteriaQuery cq = em.getCriteriaBuilder().createQuery();
            cq.select(cq.from(SeccionSubSeccion.class));
            Query q = em.createNamedQuery("SeccionSubSeccion.findRoots", SeccionSubSeccion.class);
            return q.getResultList();
        } finally {
            em.close();
        }
