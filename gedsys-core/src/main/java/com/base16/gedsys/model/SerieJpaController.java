...
public class SerieJpaController implements Serializable {
    ...
    public List<Serie> findSerieBySeccionSubSeccion(SeccionSubSeccion seccionSubSeccion) {
        EntityManager em = getEntityManager();
        try {
            CriteriaQuery cq = em.getCriteriaBuilder().createQuery();
            cq.select(cq.from(Serie.class));
            Query q = em.createNamedQuery("Serie.findBySeccionSubSeccion", Serie.class)
                    .setParameter("seccionSubseccion", seccionSubSeccion);
            return q.getResultList();
        } finally {
            em.close();
        }
